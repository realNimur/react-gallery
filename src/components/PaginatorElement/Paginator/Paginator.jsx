...
const Paginator = function Paginator({ totalPaintings }) {
  const dispatch = useDispatch();
  const { limitPaintings: limit, request, themeColor } = useSelector((store) => store);
  const { page: currentPage } = request;
  const { pages, totalPages } = usePaginator(totalPaintings, limit, currentPage);
  ...
  return (
    <ul className={cn(styles['paginator-list'], themeColor === 'white' && styles['paginator-list_white'])}>
      ...
      <button
        type="button"
        className={cn(styles['paginator-list__arrow'], currentPage === 1 && styles.disabled)}
        onClick={() => currentPage !== 1 && dispatch(setQueryPage(currentPage - 1))}
      >
      ...
      <button
        ...
        onClick={() => {
          if (currentPage !== totalPages) dispatch(setQueryPage(currentPage + 1));
        }}
      >
        <svg width="9" height="13" viewBox="0 0 9 13" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
      ...
      <button
        ...
        className={cn(styles['paginator-list__arrow'], currentPage === totalPages && styles.disabled)}
        onClick={() => {
          if (currentPage !== totalPages) dispatch(setQueryPage(totalPages));
        }}
      >
        <svg width="14" height="13" viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
