...
import { toggleColorTheme } from '../../../store/filters/actions';
...
const Header = function Header() {
  ...
  const oppositeColorTheme = colorTheme === 'white' ? 'black' : 'white';
  const handlerToggleTheme = () => {
    localStorage.setItem('themeColor', oppositeColorTheme);
    dispatch(toggleColorTheme());
  };

  useEffect(() => {
    const savedTheme = localStorage.getItem('themeColor');
    if (savedTheme === 'white') dispatch(toggleColorTheme());
    // eslint-disable-next-line
  }, []);
  return (
    <header className={styles.header}>
      <a href="/react-gallery" className={styles.header__logo}>
        <img src={logo} alt="logo" />
      </a>
      <button type="button" className={styles.header__brightness} onClick={handlerToggleTheme}>
        <svg
          preserveAspectRatio="xMinYMin meet"
          viewBox="0 0 20 20"
          fill={oppositeColorTheme}
